 ...
 * 
 * Modular test harness for scripts/build.js.
 * - Smoke tests cover helper utilities with synthetic fixtures
 * - Build matrix tests verified assistants from build-config.json
 * - Temporary workspaces keep runs isolated
 * 
 * @module build.test
 */
...
/**
 * Manually verified assistant IDs
 * @constant {Set<string>}
 */
const MANUALLY_VERIFIED_ASSISTANT_IDS = new Set([
  'copilot', 'gemini', 'cursor', 'windsurf', 'roo', 
  'qwen', 'opencode', 'kilocode', 'auggie', 'claude'
]);

/**
 * Expectation overrides for assistants with non-default behavior
 * @constant {Object}
 */
...
/**
 * Validates markdown command content
...
/**
 * Validates TOML command content
 * @param {string} content - File content
 * @param {Object} expectations - Expected values
 */
...
/**
 * Asserts command content based on format
 * @param {string} content - File content
 * @param {Object} expectations - Expected values
 */
...
/**
 * Resets the temporary root directory
 */
...
/**
 * Executes a function within a temporary directory context
 * @param {string} name - Temp directory prefix
...
/**
 * Creates test fixtures for build tests
 * @param {string} dir - Directory path
 * @param {Object} options - Fixture options
 * @returns {Promise<string>} Source directory path
 */
...
// Setup and teardown
beforeAll(async () => {
  console.log('\n[INFO] APM Build Script - Unit Tests');
