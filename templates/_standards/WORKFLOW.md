...
**Execution flow** - The Worker integrates dependency context if present, executes steps sequentially, then validates per the Task Prompt's validation criteria. The Worker validates autonomously first (running checks, verifying outputs), then pauses when criteria require User involvement (judgment or action) - the Worker does not involve the User until autonomous checks pass. When validation fails, the Worker attempts a correction. If the initial correction does not resolve the issue, the Worker spawns a debug subagent for fresh-context investigation before continuing to iterate in the main context. When a Task includes subagent steps, the Worker spawns the relevant subagent and integrates findings into execution.
