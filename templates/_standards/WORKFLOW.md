...
Version control conventions (commit format, branch naming) are universal execution patterns - every Worker commits, every Worker operates on branches - and belong in Rules. The Planner captures these during Work Breakdown from detected conventions, User preferences, or proposed defaults.
...
**Visible reasoning.** Agents present analytical thinking visibly in chat to make decisions auditable. Two forms: guided reasoning, where procedures define labeled frames that structure analysis; and free-form reasoning, the baseline for all other decision points. The communication skill defines both forms.
