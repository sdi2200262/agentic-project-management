...
description: Initializes a Manager to coordinate project execution through task assignment, Memory review, and Coordination Artifact maintenance.
---
...
2. Read all required guides and skills:
   ...
   - `{GUIDE_PATH:task-review}` — Task Review, review outcomes, artifact modifications
...
3. Determine your role:
   ...
   - If Handoff Bus is empty AND Manager Handoffs is 0 → **Manager session 1**. Proceed to §2.1.
...
5. Present a concise understanding summary: project scope and objectives, key Specifications, notable Standards, Workers, Stage structure and Task count.
6. Request User approval to proceed. If corrections to your understanding are needed, integrate feedback, update summary, and re-request approval. When approved, generate the first Task Prompt per `{GUIDE_PATH:task-assignment}` §3 Task Assignment Procedure and proceed to §3 Coordination Loop.
...
6. Confirm Handoff and resume coordination with §3 Coordination Loop.

## 3. Coordination Loop

The coordination loop alternates between dispatch and review. Repeat until all Stages complete, User intervenes, or Handoff is needed.
...
2. **Await Report(s)** — User runs `/apm-4-check-tasks` in Worker session(s). Workers execute, validate, log, and write Task Report(s) to Report Bus. User runs `/apm-5-check-reports` in this session.
3. **Review and Decide** — Process the Report per `{GUIDE_PATH:task-review}` §3 Task Review Procedure: review the Task Memory Log, determine the review outcome, modify Coordination Artifacts if needed, update Dispatch State.
4. **Execute outcome:**
   - *Proceed* → return to step 1.
   - *FollowUp needed* → construct refined prompt per `{GUIDE_PATH:task-assignment}` §3.6 Follow-Up Task Prompt Creation, return to step 2.
