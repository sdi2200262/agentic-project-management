...
1. **Determine scan scope:**
   - If `{ARGS}` argument is provided → resolve agent-id per `{SKILL_PATH:apm-communication}` §2.6 Resolving Agent IDs. Check only `.apm/bus/<agent-slug>/apm-report.md`. Proceed to step 3.
