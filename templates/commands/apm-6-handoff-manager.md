...
2. Create Handoff Log per §3 Handoff Log Structure, capturing **past actions** - what WAS done:
   - Session-level coordination: Stages managed, Tasks reviewed, dispatch cycles completed.
...
2. Present both artifacts to User: Handoff Log (file path) and handoff prompt (bus path). Request review and direct User to start a new session using `/apm-2-initiate-manager` - the incoming Manager will auto-detect the handoff prompt.
...
**Body:**
- *Title:* `# Manager Handoff <N> (Session <N> → <N+1>)`. Each section uses `##` heading.
- *Session Summary:* Stages coordinated, Tasks reviewed, dispatch cycles completed.
