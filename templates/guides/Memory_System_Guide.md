...
## 1. Overview
This guide defines how APM Manager Agents maintain the APM Memory System during project execution. The Memory System uses a **Dynamic-MD** architecture to store project history as structured Markdown files. The Manager Agent is responsible for maintaining this system throughout the APM session.
...
**Phase Summaries:** Appended after each phase completion. See §3.5 Memory Root Management.
...
**Naming Convention:** Use task ID and title from Implementation Plan, excluding agent assignment. Example: Task `Task 2.1 - Deploy Updates | Agent_Backend` → `Task_02_01_Deploy_Updates.md`
