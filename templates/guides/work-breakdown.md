...
This guide defines the methodology for Work Breakdown, which transforms gathered context into Coordination Artifacts through Chain-of-Thought reasoning — thinking visibly in chat before committing to files.
...
### 2.1 Chain-of-Thought Methodology
...
Complete each step before proceeding to the next. Present reasoning in chat before writing files per §2.1 Chain-of-Thought Methodology.
...
3. Domain Analysis → identify Worker Agents, update header
...
2. Update `.apm/Specifications.md`:
   ...
   - Fill **Last Modification** field: "Specifications creation by the Planner Agent."
...
1. Present domain organization reasoning in chat:
   ...
   - **Agent mapping** — how domains map to Worker Agents with proposed names and responsibilities.
2. Update Implementation Plan header Agents field.

### 3.4 Stage Analysis

Apply §2.2 Decomposition Principles. Identify all Stages and their Tasks upfront — detailed Task breakdown happens in §3.5 Stage Cycle Protocol. Perform the following actions:
...
After all three checkpoints are approved, output procedure completion stating Work Breakdown is complete, all Coordination Artifacts are created, and the next step is initializing the Manager Agent using `/apm-2-initiate-manager`.
