...
1. Initialize Memory Root per `{SKILL_PATH:memory-maintenance/SKILL.md}` §3.1 Memory Root Initialization.
...
5. Create Stage 1 directory per `{SKILL_PATH:memory-maintenance/SKILL.md}` §3.2 Stage Directory Creation.
6. Generate the first Task Assignment Prompt per `{SKILL_PATH:task-assignment/SKILL.md}` §3 Task Assignment Procedure and output as markdown code block. Proceed to §3 Task Cycle.
...
1. Create Stage Summary per `{SKILL_PATH:memory-maintenance/SKILL.md}` §3.6 Stage Summary Creation.
...
- **Authority thresholds:** Follow `{SKILL_PATH:artifact-maintenance/SKILL.md}` §2.3 Modification Authority Standards for Coordination Artifact modifications requiring User collaboration
